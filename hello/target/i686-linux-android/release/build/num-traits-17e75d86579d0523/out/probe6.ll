; ModuleID = 'probe6.e167d3b24b8af7a2-cgu.0'
source_filename = "probe6.e167d3b24b8af7a2-cgu.0"
target datalayout = "e-m:e-p:32:32-p270:32:32-p271:32:32-p272:64:64-f64:32:64-f80:32-n8:16:32-S128"
target triple = "i686-unknown-linux-android"

@alloc_d4a7707f05e5d7266a36152d39817482 = private unnamed_addr constant <{ [8 x i8] }> <{ [8 x i8] c"\00\00\00\00\00\00\F0?" }>, align 4
@alloc_6548de77921b239bafa46fd51b17f832 = private unnamed_addr constant <{ [8 x i8] }> <{ [8 x i8] c"\00\00\00\00\00\00\00@" }>, align 4

; core::f64::<impl f64>::total_cmp
; Function Attrs: inlinehint uwtable
define internal i8 @"_ZN4core3f6421_$LT$impl$u20$f64$GT$9total_cmp17hd30272ff74419fadE"(ptr align 4 %self, ptr align 4 %other) unnamed_addr #0 {
start:
  %right = alloca i64, align 4
  %left = alloca i64, align 4
  %_0 = alloca i8, align 1
  %self1 = load double, ptr %self, align 4, !noundef !2
  %_4 = bitcast double %self1 to i64
  store i64 %_4, ptr %left, align 4
  %self2 = load double, ptr %other, align 4, !noundef !2
  %_7 = bitcast double %self2 to i64
  store i64 %_7, ptr %right, align 4
  %_13 = load i64, ptr %left, align 4, !noundef !2
  %_12 = ashr i64 %_13, 63
  %_10 = lshr i64 %_12, 1
  %0 = load i64, ptr %left, align 4, !noundef !2
  %1 = xor i64 %0, %_10
  store i64 %1, ptr %left, align 4
  %_18 = load i64, ptr %right, align 4, !noundef !2
  %_17 = ashr i64 %_18, 63
  %_15 = lshr i64 %_17, 1
  %2 = load i64, ptr %right, align 4, !noundef !2
  %3 = xor i64 %2, %_15
  store i64 %3, ptr %right, align 4
  %_22 = load i64, ptr %left, align 4, !noundef !2
  %_23 = load i64, ptr %right, align 4, !noundef !2
  %_21 = icmp slt i64 %_22, %_23
  br i1 %_21, label %bb1, label %bb2

bb2:                                              ; preds = %start
  %_25 = load i64, ptr %left, align 4, !noundef !2
  %_26 = load i64, ptr %right, align 4, !noundef !2
  %_24 = icmp eq i64 %_25, %_26
  br i1 %_24, label %bb3, label %bb4

bb1:                                              ; preds = %start
  store i8 -1, ptr %_0, align 1
  br label %bb6

bb4:                                              ; preds = %bb2
  store i8 1, ptr %_0, align 1
  br label %bb5

bb3:                                              ; preds = %bb2
  store i8 0, ptr %_0, align 1
  br label %bb5

bb5:                                              ; preds = %bb3, %bb4
  br label %bb6

bb6:                                              ; preds = %bb1, %bb5
  %4 = load i8, ptr %_0, align 1, !range !3, !noundef !2
  ret i8 %4
}

; probe6::probe
; Function Attrs: uwtable
define void @_ZN6probe65probe17h934650beea2aa9c6E() unnamed_addr #1 {
start:
; call core::f64::<impl f64>::total_cmp
  %_1 = call i8 @"_ZN4core3f6421_$LT$impl$u20$f64$GT$9total_cmp17hd30272ff74419fadE"(ptr align 4 @alloc_d4a7707f05e5d7266a36152d39817482, ptr align 4 @alloc_6548de77921b239bafa46fd51b17f832), !range !3
  ret void
}

attributes #0 = { inlinehint uwtable "probe-stack"="inline-asm" "target-cpu"="pentiumpro" "target-features"="+mmx,+sse,+sse2,+sse3,+ssse3" }
attributes #1 = { uwtable "probe-stack"="inline-asm" "target-cpu"="pentiumpro" "target-features"="+mmx,+sse,+sse2,+sse3,+ssse3" }

!llvm.module.flags = !{!0}
!llvm.ident = !{!1}

!0 = !{i32 8, !"PIC Level", i32 2}
!1 = !{!"rustc version 1.76.0 (07dca489a 2024-02-04)"}
!2 = !{}
!3 = !{i8 -1, i8 2}
